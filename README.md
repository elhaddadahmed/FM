...
Transaktionen automatisch synchronisieren) — Zugriffstoken liegen dabei
ausschließlich in einer Supabase Edge Function, nie im Browser-Code.

Setup: siehe `OPEN-BANKING-SETUP.md` (zusätzlich `open-banking-schema.sql`
ausführen und die Edge Function `open-banking` mit deinen GoCardless-Secrets
deployen). Ohne dieses Setup bleibt der manuelle CSV-Kontoauszug-Import
(Konten-Tab → "Kontoauszug importieren", erkennt Sparkasse/Volksbank/VR/
